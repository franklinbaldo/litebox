...
pub struct CliArgs {
    ...
    /// Target OS for the rewritten binaries (default: host OS).
    /// On ARM64, each OS has distinct rewriting behavior (TLS mechanism, x18
    /// handling, trampoline layout). On x86, this has no effect.
    #[arg(long = "target-os", value_enum)]
    ...
    let par_results: Vec<anyhow::Result<Vec<TarEntry>>> = file_map_vec
        ...
        .map(|(real_path, tar_paths)| {
            ...
            let rewritten = if no_rewrite.contains(real_path) {
                ...
                rewrite_elf(&data, real_path, verbose, target_os)?
            };
    ...
    let par_results: Vec<anyhow::Result<TarEntry>> = file_entries
        ...
        .map(|(_key_path, entry)| {
            ...
            let rewritten = if entry.is_executable && !no_rewrite.contains(&entry.read_path) {
                rewrite_elf(&data, &entry.read_path, verbose, target_os)?
            } else {
                data
            };
...
fn rewrite_elf(
    ...
    match litebox_syscall_rewriter::hook_syscalls_in_elf(data, None, target_os) {
        Ok(rewritten) => {
            if verbose {
