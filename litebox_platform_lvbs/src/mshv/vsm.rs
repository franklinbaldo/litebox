...
use crate::{
    ...
    mshv::{
        ...
        HvPageProtFlags, HvRegisterVsmPartitionConfig, HvRegisterVsmVpSecureVtlConfig, VsmError,
...
/// Install the Hyper-V CR-intercept for the control registers HEKI protects and
/// snapshot their current VTL0 values (used by the intercept handler). The
/// register set is fixed here as an enforcement detail; HEKI decides *when* to
/// lock via [`crate::heki_adapter`].
pub(crate) fn lock_control_registers() -> Result<(), VsmError> {
...
fn protect_vtl1_physical_memory_range(
    ...
    let num_pages = phys_frame_range.count() as u64;
    if num_pages > 0 {
        hv_modify_vtl_protection_mask(pa, num_pages, HvPageProtFlags::empty())
            .map_err(VsmError::HypercallFailed)?;
    }
    Ok(())
}
