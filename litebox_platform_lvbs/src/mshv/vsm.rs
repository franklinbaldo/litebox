...
pub(crate) struct FrameReservation<'a> {
    /// Needed by `Drop`, which cannot take arguments, so the reservation borrows the
    /// platform rather than looking it up ambiently at rollback time. A plain borrow
    /// suffices: a reservation is always a local, never stored.
...
/// `'static` is not incidental: [`Vtl0Gate::install_ringbuffer`] parks the
/// platform in the process-wide ring buffer, and the runner stores this gate in
/// the long-lived HEKI service. Either alone would force it.
...
/// Held by the runner.
/// `'static` because two of its operations mint a [`LvbsVtl0Gate`], which is
/// itself `'static`.
...
impl LvbsVtl0Gate {
    ...
    /// composition-root code (the runner), which supplies the platform.
    #[must_use]
