...
//! Platform-side implementation of the [`litebox_common_lvbs::VsmBackend`] trait.

use alloc::vec::Vec;
use litebox_common_linux::vmap::PhysPageAddr;
use litebox_common_lvbs::{
    FrameTxn, HypervCallError, MemAttr, PAGE_SIZE, PRK_LEN, ReservationStatus, VsmBackend, VsmError,
};
use x86_64::{
    PhysAddr,
...
/// Zero-sized live adapter implementing [`VsmBackend`] over the platform's
/// enforcement primitives.
pub struct LvbsVsmBackend;

/// Maps a [`MemAttr`] permission set (the VsmBackend permission type) to the
...
impl VsmBackend for LvbsVsmBackend {
    fn read_vtl0_bytes(
        &self,
