...
impl SyscallRequest {
    pub(crate) fn try_from_raw(pt_regs: &litebox_common_linux::PtRegs) -> Option<Self> {
        ...
        macro_rules! sys_req {
            ($id:ident { $( $field:ident $(: { $expr:expr })? ),* $(,)? }) => {
                sys_req!(@[$id] [ $( $field $(: { $expr })? ),* ] [ 0, 1, 2, 3 ] [ ])
            };
            (@[$id:ident] [ $f:ident $(,)? $($field:ident $(: { $expr:expr })?),* ] [ $n:literal $(,)? $($ns:literal),* ] [ $($tail:tt)* ]) => {
                sys_req!(@[$id] [ $( $field $(: { $expr })? ),* ] [ $($ns),* ] [ $($tail)* $f: pt_regs.win_sys_req_arg($n), ])
            };
            (@[$id:ident] [ $f:ident : { $expr:expr } $(,)? $($field:ident $(: { $expr_tail:expr })?),* ] [ $n:literal $(,)? $($ns:literal),* ] [ $($tail:tt)* ]) => {
                sys_req!(@[$id] [ $( $field $(: { $expr_tail })? ),* ] [ $($ns),* ] [ $($tail)* $f: ($expr)(pt_regs.win_sys_req_arg($n)), ])
            };
