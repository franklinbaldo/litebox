...
pub fn prepare_image_bundle(
    ...
    // Always (re-)write /etc/resolv.conf with the host's current DNS servers.
    // The host DNS can change between runs (different networks, VPN, WSL2
    // gateway, etc.), so we refresh every time rather than only at first cache.
...
/// resolver (127.0.0.11) that won't work in our sandbox.  We prefer the
/// host's own nameservers (read from `/etc/resolv.conf`) so that the
/// broker's network proxy relays DNS through the same resolver the host
/// uses — this is important on WSL2 where public DNS like 8.8.8.8 may be
/// unreachable due to NAT.  Falls back to 8.8.8.8 / 1.1.1.1 when the
/// host file cannot be read.
///
/// If the rootfs file was provided by the image (not generated by us) and
/// contains a valid non-localhost nameserver, we leave it alone.  Our own
/// generated files (tagged with "Generated by litebox-oci") are always
/// refreshed with current host DNS.
fn ensure_resolv_conf(rootfs_dir: &Path) -> Result<()> {
    let etc_dir = rootfs_dir.join("etc");
    let resolv_path = etc_dir.join("resolv.conf");

    // If it already exists, was NOT written by us, and has a working
    // nameserver, leave the image's resolv.conf alone.
    if resolv_path.exists()
        && let Ok(content) = fs::read_to_string(&resolv_path)
    {
        let is_ours = content.contains("Generated by litebox-oci");
        if !is_ours {
    ...
    // Read the host's nameservers from /etc/resolv.conf so the guest uses
    // the same DNS resolver as the host.  This avoids issues on WSL2 where
    // the host DNS is on the Hyper-V gateway (e.g. 172.x.x.x) and public
    // servers may be unreachable.
    let nameservers = read_host_nameservers();
    let ns_content = if nameservers.is_empty() {
        "# Generated by litebox-oci (fallback)\nnameserver 8.8.8.8\nnameserver 1.1.1.1\n"
            .to_string()
    } else {
        let mut buf = String::from("# Generated by litebox-oci (from host)\n");
        for ns in &nameservers {
            let _ = writeln!(buf, "nameserver {ns}");
        }
        buf
    };

    let display_ns: Vec<&str> = ns_content
        .lines()
        .filter_map(|l| l.strip_prefix("nameserver "))
        .collect();

    fs::write(&resolv_path, &ns_content).context("failed to write /etc/resolv.conf in rootfs")?;

    eprintln!("[BUILD]   wrote /etc/resolv.conf (DNS: {})", display_ns.join(", "));
    Ok(())
}

/// Read non-localhost nameservers from the host system.
///
/// - **Linux / macOS / WSL2**: parses `/etc/resolv.conf`.
/// - **Windows**: runs `powershell Get-DnsClientServerAddress` and extracts
///   IPv4 addresses.
/// - Falls back to an empty vec (caller uses 8.8.8.8 / 1.1.1.1).
fn read_host_nameservers() -> Vec<String> {
    // Try /etc/resolv.conf first (Linux, macOS, WSL2).
    ...
    // On macOS, scutil can also provide DNS info.
        ...
        let servers: Vec<String> = String::from_utf8_lossy(&output.stdout)
            ...
            .filter_map(|line| {
                ...
                if let Some(addr) = line.strip_prefix("nameserver[") {
                    ...
                    addr.split(':').nth(1).map(|s| s.trim().to_string())
                } else {
                    None
                }
            })
