...
/// [`crate::physical_pointers::PhysConstPtr`], which borrow an implementor for as long as they
/// live. It can benefit other modules which need Linux kernel's `vmap()` and `vunmap()`
