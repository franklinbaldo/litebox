...
mod tests {
    ...
    use super::{AccessMemory, Fault, PeImageInfo, PeLoadError, PeParseError, PeParsedFile};
    use object::pe;

    #[test]
    fn rejects_non_pe_bytes() {
        let err = PeParsedFile::parse_bytes(&[0; 64]).unwrap_err();
        assert!(matches!(err, PeParseError::Pe(_)));
    }

    #[test]
    fn rejects_empty_relocations_when_loaded_away_from_preferred_base() {
        let parsed = parsed_with_characteristics();
        let mut mem = DummyMemory;

        let result: Result<(), PeLoadError<Infallible>> =
            parsed.apply_base_relocations(0x1000, &mut mem);

        assert!(matches!(result, Err(PeLoadError::RelocationRequired)));
    }

    fn parsed_with_characteristics() -> PeParsedFile {
        PeParsedFile {
            image: PeImageInfo {
    ...
    impl AccessMemory for DummyMemory {
        fn read(&mut self, _address: usize, _buf: &mut [u8]) -> Result<(), Fault> {
            unreachable!()
        }
