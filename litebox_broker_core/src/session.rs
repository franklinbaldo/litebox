...
use crate::timerfd::TimerfdObject;
    ...
    pub fn check_readiness(&self, handle: ObjectHandle) -> Result<ReadinessFlags> {
        ...
        let socket = {
            ...
            match &*object {
                ...
                ObjectEntry::Timerfd(timerfd) => return Ok(timerfd.resource().readiness()),
...
impl Drop for BrokerSession {
    fn drop(&mut self) {
        ...
        self.core.timerfd_provider.close_session(self.session_id);
    }
}
...
mod tests {
    ...
    fn object_reference_lifecycle_uses_public_core_constructor_once() {
        ...
        let timerfd_provider = Arc::new(crate::timerfd::tests::TestTimerfdProvider::default());
        ...
        .unwrap()
        .with_timerfd_provider(timerfd_provider.clone());
        ...
        crate::timerfd::tests::check_timerfd_lifecycle(&broker, &timerfd_provider);
