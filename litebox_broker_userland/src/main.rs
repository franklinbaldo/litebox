...
use litebox_broker_platform_linux_userland::{
    LinuxSocketProvider, SocketPortMapping, SocketPortMappingProtocol,
};
...
struct CliArgs {
    ...
    /// Publish a host IPv4 UDP endpoint to a guest-local UDP port.
    #[arg(long, value_name = "HOST_IP:HOST_PORT:GUEST_PORT")]
    publish_udp: Vec<PortMappingArgument>,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let port_mappings = configured_port_mappings(&args.publish_tcp, &args.publish_udp);
...
fn configured_port_mappings(
    tcp: &[PortMappingArgument],
    udp: &[PortMappingArgument],
    ...
    tcp.iter()
        ...
        .chain(udp.iter().map(|mapping| SocketPortMapping {
