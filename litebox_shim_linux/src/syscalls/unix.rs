...
impl<FS: ShimFS> UnixStream<FS> {
    ...
    fn lookup(&self, task: &Task<FS>, addr: &UnixSocketAddr) -> Result<Arc<Backlog<FS>>, Errno> {
        ...
        let Some(entry) = guard.get(&key) else {
            // Log Unix socket connect failures for diagnostics.
            let table_keys: alloc::vec::Vec<_> = guard.keys().collect();
            let msg = alloc::format!(
                "UNIX CONNECT REFUSED: path={:?} table_size={} keys={:?} pid={} tid={}\n",
                key,
                guard.len(),
                table_keys,
                task.process_id.0,
                task.process_id.0, // use process_id for both since thread_id is not a public field
            );
            use litebox::platform::DebugLogProvider as _;
            litebox_platform_multiplex::platform().debug_log_print(&msg);
