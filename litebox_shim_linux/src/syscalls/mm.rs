...
impl<FS: ShimFS> Task<FS> {
    ...
    fn try_cow_mmap_file(
        ...
        let static_data = files
            .run_on_raw_fd(
                raw_fd,
                |typed_fd| files.fs.get_static_backing_data(&files.dt, typed_fd),
                |_| None,
                |_| None,
                |_| None,
                |_| None,
        ...
        match <_ as PageManagementProvider<{ PAGE_SIZE }>>::try_allocate_cow_pages(
            ...
            Ok(ptr) => {
                ...
                // `permissions`.
                unsafe {
                    self.pm.register_existing_mapping(
                        range,
                        permissions,
                        true,
    ...
    pub(crate) fn sys_munmap(&self, addr: crate::MutPtr<u8>, len: usize) -> Result<(), Errno> {
        litebox_common_linux::mm::sys_munmap(&self.pm, addr, len)
    }
    ...
    pub(crate) fn sys_mprotect(
        ...
        litebox_common_linux::mm::sys_mprotect(&self.pm, addr, len, prot)
    }
    ...
    pub(crate) fn sys_brk(&self, addr: MutPtr<u8>) -> Result<usize, Errno> {
        litebox_common_linux::mm::sys_brk(&self.pm, addr)
    }
    ...
    pub(crate) fn sys_madvise(
        ...
        litebox_common_linux::mm::sys_madvise(&self.pm, addr, len, advice)
    }
}
