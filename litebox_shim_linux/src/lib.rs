...
impl<FS: ShimFS> LinuxShimEntrypoints<FS> {
    ...
    /// `FdKind` variants fail to compile here.
    ///
    ...
    pub fn install_broker_bridge_fd(
        &self,
        guest_fd: usize,
        snapshot: syscalls::fork_snapshot::FdKind,
        pipe_flags: litebox::fs::OFlags,
    ) -> Result<(), ()> {
        use syscalls::fork_snapshot::FdKind;
        let files = self.task.files.borrow();
        match snapshot {
            FdKind::Eventfd { handle_id } => {
            ...
            FdKind::Signalfd { handle_id } => todo!(
                "install_broker_bridge_fd for FdKind::Signalfd \
...
impl<FS: ShimFS> LinuxShim<FS> {
    ...
    pub fn restore_process(
        ...
        // Restore snapshot fd entries from the canonical fd-kind taxonomy.
        {
            ...
            for entry in &fd_table.entries {
                match entry.kind {
                    FdKind::FilesystemFd => {
                        if entry.fd <= 2
                            && fd_table.stdio_object_ids.contains(&Some(entry.object_id))
                        {
                            continue;
                        }
                    ...
                    FdKind::UnixSocket => {
                        ...
                        if let Some(socket) = syscalls::unix::UnixSocket::<FS>::new(
                            ...
                            debug_assert!(
                                success,
                                "unix fd slot {} occupied during restore",
                                entry.fd
                            );
                        }
                    }
                    FdKind::Epoll | FdKind::Inotify => {
                        // Not snapshot-restored; migrated via `--broker-fd-bridge` specs installed later.
                    }
                    FdKind::HostPassthrough {
                        ...
                        let Some(client) = litebox_common_linux::fd_token_client::global_client()
                        else {
                            continue;
                        };
                        let Ok(raw_fd) = client.materialize_raw_fd(token_id) else {
                            let _ = client.release(token_id);
                            continue;
                        };
                        let _ = client.release(token_id);
                        let host_entry = syscalls::host_passthrough_fd::HostPassthroughFdEntry::new(
                            raw_fd, direction,
                        );
                        let typed = self
                            .global
                            .litebox
                            .descriptor_table_mut()
                            .insert::<syscalls::host_passthrough_fd::HostPassthroughFd>(
                            host_entry,
                        );
                        let mut rds = child_files.raw_descriptor_store.write();
                        if entry.fd <= 2 {
                    ...
                    #[cfg(feature = "worker_local_inet")]
                    FdKind::Net => {
                        // Local-worker inet sockets are not snapshot-restored by this path.
                    }
                    FdKind::Eventfd { handle_id } => {
                    ...
                    FdKind::BrokerPipe {
                        ...
                        // FdKind::Pipe slots whose snapshot carries a broker_handle.
                        ...
                        //
                        // Now: per FdKind::Pipe entry with broker_handle, create a
                    ...
                    FdKind::BrokerPty {
                        ...
                        let Ok(pty_fd) =
                            syscalls::broker_pty::BrokerPtyProviderReacquireExt::reacquire(
                                &provider, handle_id, role, pty_id,
                            )
                        else {
                            continue;
                    ...
                    FdKind::BrokerSocketDgram { handle_id } => {
                        ...
                        let _ = releaser.dup_handle(handle_id);
                        let dgram_fd =
                            syscalls::broker_socket_dgram::BrokerSocketDgramFd::<Platform>::new(
                                provider,
                                handle_id,
                    ...
                    FdKind::BrokerInetDgram { .. } | FdKind::BrokerInetRaw { .. } => {
                        // Broker-backed inet datagram/raw fds are not migrated by this restore path today.
                    }
                }
            }
        }
...
impl<FS: ShimFS> Task<FS> {
    ...
    fn restore_cow_layer(&self, cow: &Arc<CowState>, restore_bytes: bool) {
        ...
        if restore_bytes {
            ...
            // `pthread_join` waiting on the resurrected futex forever. Writing
            // 0 and waking the futex lets the join complete. If a lower layer
            // still covers the address it will roll it back again on its own
            // restore, so hand the pending wake down to that layer instead.
            let pending = core::mem::take(&mut *cow.pending_join_wakes.lock());
            for addr in pending {
                if let Some((lower, _)) = self.top_cow_layer_for_page(addr) {
                    lower.pending_join_wakes.lock().push(addr);
                    continue;
                }
...
struct CowState {
    ...
    /// `clear_child_tid` addresses of sibling threads that *exited during this
    /// vfork window*. Such a thread is allowed to bypass the fork quiesce (an
    /// exiting thread must make progress so the forker isn't wedged waiting for
    /// it to park), so it writes `0` to its `clear_child_tid` — the futex a
    /// `pthread_join` waits on — inside the window. `restore_cow_layer` then
    /// rolls that page back to the pre-clear (stale TID) value, resurrecting the
    /// join futex and wedging the joiner forever. We record the address here and
    /// re-apply the clear+wake after the byte restore so the join can complete.
    pending_join_wakes: litebox::sync::Mutex<Platform, Vec<usize>>,
}
...
mod tests {
    ...
    fn install_broker_unix_socket_replaces_restored_unix_socket_slot() {
        ...
        assert_eq!(
            entrypoints.install_broker_bridge_fd(
                guest_fd,
                syscalls::fork_snapshot::FdKind::BrokerSocketPair {
                    handle_id: 1234,
    ...
    fn install_broker_pipe_dup_handle_failure_leaves_slot_empty() {
        ...
        assert_eq!(
            entrypoints.install_broker_bridge_fd(
                guest_fd,
                syscalls::fork_snapshot::FdKind::BrokerPipe {
