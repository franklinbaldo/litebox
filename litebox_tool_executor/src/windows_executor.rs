...
//! Windows-specific in-process sandbox execution via the Windows userland platform.
...
/// Execute a command inside a LiteBox sandbox (Windows in-process).
///
...
pub fn execute(
    ...
    // echoes keystrokes by default. The guest shell also echoes input,
    // causing double-echo. We switch to raw mode so only the guest's echo
    // is visible, matching how a Linux terminal works.
