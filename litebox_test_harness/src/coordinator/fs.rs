...
pub(super) async fn fs_tests(r: &mut TestRunner) {
    ...
    if matches!(&resp, Response::NotFound) {
        r.record_xfail("F6.host→init", "init", pass, "host_wrote.txt not in rootfs", &format!("{resp:?}"));
    } else {
    ...
    let pass = matches!(&resp, Response::Ok { data: Some(d) } if d == "from_host");
    if matches!(&resp, Response::NotFound) {
        r.record_xfail("F6.host→A", "A", pass, "host_wrote.txt not in rootfs", &format!("{resp:?}"));
    } else {
