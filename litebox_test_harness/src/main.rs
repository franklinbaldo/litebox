...
fn main() {
    ...
    match cmd {
        ...
        // ── Minimal canary repros (M1–M4) ─────────────────────────────────
        // These exist to isolate the cascade root identified in
        // wave-0 discovery: agent A's tokio runtime calls
        // `tokio::process::Command::spawn(<non-PIE>)` (during
        // spawn_tree's SpawnRemote NP) and dies as a side effect.
        //
        // Each subcommand spawns a non-PIE child by a different
        // mechanism, then verifies the parent process is still alive
        // and functional after the wait. Native must pass all four;
        // litebox failure patterns help triangulate the bug:
        //
        //   M1 fails, M2 passes  ⇒ tokio-specific (likely the
        //                          tokio worker thread getting
        //                          exit_thread'd from
        //                          exec_on_remote_host)
        //   M1 passes, M2 fails  ⇒ raw libc fork+execve broken too
        //   M3 fails after M1 passes ⇒ parent "almost dead" — some
        //                              threads exited but main
        //                              survived briefly
        //   M4 fails at iteration K  ⇒ parent survives K spawns
        //                              before dying
        "M1-tokio-spawn-nonpie" => {
            ...
            let nonpie = find_nonpie_binary().unwrap_or_else(|| {
                eprintln!("[M1] FAIL: nonpie binary not found");
                println!("M1_FAIL:no_nonpie_binary");
                std::process::exit(1);
            });
        ...
        "M2-libc-spawn-nonpie" => {
            ...
            let nonpie = find_nonpie_binary().unwrap_or_else(|| {
                eprintln!("[M2] FAIL: nonpie binary not found");
                println!("M2_FAIL:no_nonpie_binary");
                std::process::exit(1);
            });
            let parent_pid = std::process::id();
        ...
        "M3-tokio-spawn-nonpie-then-work" => {
            ...
            // catches it.
            let nonpie = find_nonpie_binary().unwrap_or_else(|| {
                println!("M3_FAIL:no_nonpie_binary");
                std::process::exit(1);
            });
            let parent_pid = std::process::id();
        ...
        "M4-tokio-spawn-nonpie-repeated" => {
            ...
            let nonpie = find_nonpie_binary().unwrap_or_else(|| {
                println!("M4_FAIL:no_nonpie_binary");
                std::process::exit(1);
            });
            let parent_pid = std::process::id();
