...
//! VSM policy (module/kexec validation, HEKI patching, kernel data) algorithms.
//!
//! These algorithms are generic over the [`crate::HekiEnforcer`] port and take an
//! explicit [`HekiState`] context, so they are host-testable with no platform.
...
use crate::{HekiEnforcer, ReservationStatus};
...
use litebox_common_lvbs::{
    ...
    HekiPatchInfo, HekiRange, KEXEC_SEGMENT_MAX, Kimage, MemAttr, ModMemType, PAGE_SIZE, VsmError,
...
pub fn mshv_vsm_end_of_boot(state: &HekiState) -> i64 {
    log::debug!("VSM: End of boot");
    state.set_end_of_boot();
    0
}

/// VSM function for locking the control registers HEKI protects from VTL0
/// modification. Rejected after end of boot; the register set itself is a
/// platform enforcement detail (fixed for now).
pub fn mshv_vsm_lock_regs(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
) -> Result<i64, VsmError> {
    ...
    enforcer.lock_control_registers()?;
    Ok(0)
}
...
pub fn mshv_vsm_protect_memory(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    pa: u64,
    nranges: u64,
...
pub fn mshv_vsm_load_kdata(
    ...
    system_certs_mem
        .write_bytes_from_heki_range(enforcer)
...
pub fn mshv_vsm_validate_guest_module(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    pa: u64,
    nranges: u64,
    ...
    // adapter reserves `initial`, commits on `Ok`, and rolls back (unprotecting every newly reserved
    // range) on `Err` — preserving the original `FrameReservation`/`commit`/drop semantics exactly.
...
pub fn mshv_vsm_free_guest_module_init(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    token: i64,
) -> Result<i64, VsmError> {
        ...
        for mod_mem_range in entry.iter_mem_ranges() {
            let range_result = match mod_mem_range.mod_mem_type {
                ModMemType::InitText | ModMemType::InitData | ModMemType::InitRoData => {
                    enforcer.unprotect_frames(mod_mem_range.phys_frame_range)
                }
...
/// `token` is the unique identifier for the module.
pub fn mshv_vsm_unload_guest_module(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    token: i64,
) -> Result<i64, VsmError> {
...
pub fn mshv_vsm_kexec_validate(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    pa: u64,
    nranges: u64,
    ...
    for old_kexec_mem_range in kexec_metadata_ref.iter_guarded().iter_mem_ranges() {
        enforcer.unprotect_frames(old_kexec_mem_range.phys_frame_range)?;
    }
...
pub fn mshv_vsm_patch_text(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    patch_pa_0: u64,
    patch_pa_1: u64,
) -> Result<i64, VsmError> {
    let heki_patch = copy_heki_patch_from_vtl0(enforcer, patch_pa_0, patch_pa_1)?;
    ...
    if let Some(validated) = ValidatedTextPatch::prepare(&heki_patch, &precomputed_patch)? {
        enforcer.apply_validated_text_patch(&validated)?;
    }
    Ok(0)
}
...
/// cannot be requested (across the port) without a successful validation. Other
/// crates can read the parameters (to perform the write) but cannot forge one.
pub struct ValidatedTextPatch<'a> {
...
impl<'a> ValidatedTextPatch<'a> {
    ...
    fn prepare(patch: &'a HekiPatch, precomputed: &HekiPatch) -> Result<Option<Self>, VsmError> {
        ...
        let pa_1 = PhysAddr::new(patch.pa[1]);
        ...
        let (target_pages, page_count) = if pa_1.is_null() {
            // `HekiPatch::is_valid` guarantees a null `pa[1]` means the patch fits
            // in one page; assert it in debug builds.
            debug_assert!(
                pa_0.as_u64() + bytes.len() as u64 <= pa_0_page.as_u64() + Size4KiB::SIZE,
                "patch crosses page boundary but pa_1 is null"
            );
            ([page0, page0], 1)
        } else {
            let page1 =
                PhysPageAddr::<PAGE_SIZE>::new(pa_1.align_down(Size4KiB::SIZE).as_u64().trunc())
                    .ok_or(VsmError::Vtl0CopyFailed)?;
            ([page0, page1], 2)
        };
    ...
    #[must_use]
    pub fn pages(&self) -> &[PhysPageAddr<PAGE_SIZE>] {
...
fn copy_heki_patch_from_vtl0(
    ...
    {
        enforcer
            .read_vtl0::<HekiPatch>(patch_pa_0.as_u64().trunc())
            .map_err(|_| VsmError::Vtl0CopyFailed)
    } else {
...
pub fn mshv_vsm_allocate_ringbuffer_memory(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    phys_addr: u64,
    size: usize,
...
pub fn mshv_vsm_set_platform_root_key(
    enforcer: &impl HekiEnforcer,
    state: &HekiState,
    key_pa: u64,
) -> Result<i64, VsmError> {
    ...
    enforcer
        .read_vtl0_bytes(key_pa.as_u64().trunc(), &mut *keybuf)
        .map_err(|_| VsmError::Vtl0CopyFailed)?;
    enforcer.set_platform_root_key(&*keybuf)?;
    Ok(0)
}
...
fn copy_heki_pages_from_vtl0(
    ...
    while range < nranges {
        ...
        let heki_page = enforcer
            .read_vtl0::<HekiPage>(cur_pa.as_u64().trunc())
            .ok()?;
...
impl ModuleMemory {
    ...
    pub(crate) fn write_bytes_from_heki_range(
        ...
        self.text.write_bytes_from_heki_range(enforcer)?;
        self.init_text.write_bytes_from_heki_range(enforcer)?;
        self.init_rodata.write_bytes_from_heki_range(enforcer)?;
        Ok(())
    }
...
impl MemoryContainer {
    ...
    pub(crate) fn write_bytes_from_heki_range(
        ...
        for range in range {
            let phys_end = range
                ...
                .ok_or(MemoryContainerError::Overflow)?;
            self.write_vtl0_phys_bytes(enforcer, range.phys_addr, phys_end)?;
        }
        Ok(())
    }
    ...
    pub(crate) fn write_vtl0_phys_bytes(
        ...
        if enforcer
            .read_vtl0_bytes(phys_start.as_u64().trunc(), &mut self.buf[old_len..])
            .is_err()
        {
