...
//!
//! Each handler is generic over the [`VsmPlatform`] trait (passed as
//! `platform: &impl VsmPlatform`) and takes an explicit [`HekiState`] context
//! where needed, so the policy logic is decoupled from any concrete platform.
...
fn read_vtl0_val<P: VsmPlatform, T: FromBytes>(
    platform: &P,
    phys_addr: u64,
) -> Result<T, VsmError> {
    let mut buf = vec![0u8; core::mem::size_of::<T>()];
    read_vtl0_contiguous(platform, phys_addr, &mut buf)?;
...
pub fn mshv_vsm_enable_aps(_cpu_present_mask_pfn: u64) -> Result<i64, VsmError> {
    log::debug!("mshv_vsm_enable_aps() not supported");
    Ok(0)
}
...
pub fn mshv_vsm_boot_aps<P: VsmPlatform>(
    platform: &P,
    cpu_online_mask_pfn: u64,
) -> Result<i64, VsmError> {
...
pub fn mshv_vsm_protect_memory<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    pa: u64,
    nranges: u64,
    ...
    let heki_pages =
        copy_heki_pages_from_vtl0(platform, pa, nranges).ok_or(VsmError::HekiPagesCopyFailed)?;
...
pub fn mshv_vsm_load_kdata<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    pa: u64,
    nranges: u64,
...
pub fn mshv_vsm_validate_guest_module<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    pa: u64,
    nranges: u64,
...
pub fn mshv_vsm_free_guest_module_init<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    token: i64,
) -> Result<i64, VsmError> {
...
/// `token` is the unique identifier for the module.
pub fn mshv_vsm_unload_guest_module<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    token: i64,
) -> Result<i64, VsmError> {
...
pub fn mshv_vsm_kexec_validate<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    pa: u64,
    nranges: u64,
...
pub fn mshv_vsm_patch_text<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    patch_pa_0: u64,
    patch_pa_1: u64,
) -> Result<i64, VsmError> {
    let heki_patch = copy_heki_patch_from_vtl0(platform, patch_pa_0, patch_pa_1)?;
    ...
    if let Some(validated) = ValidatedTextPatch::prepare(&heki_patch, &precomputed_patch)? {
        platform.write_vtl0_privileged(validated.pages(), validated.offset(), validated.bytes())?;
    }
    Ok(0)
}
...
fn copy_heki_patch_from_vtl0<P: VsmPlatform>(
    platform: &P,
    patch_pa_0: u64,
    patch_pa_1: u64,
    ...
    {
        read_vtl0_val::<P, HekiPatch>(platform, patch_pa_0.as_u64())
    } else {
...
pub fn mshv_vsm_allocate_ringbuffer_memory<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    phys_addr: u64,
    size: usize,
...
pub fn mshv_vsm_set_platform_root_key<P: VsmPlatform>(
    platform: &P,
    state: &HekiState,
    key_pa: u64,
) -> Result<i64, VsmError> {
...
fn copy_heki_pages_from_vtl0<P: VsmPlatform>(
    platform: &P,
    pa: u64,
    nranges: u64,
) -> Option<Vec<HekiPage>> {
    ...
    while range < nranges {
        ...
        let heki_page = read_vtl0_val::<P, HekiPage>(platform, cur_pa.as_u64()).ok()?;
