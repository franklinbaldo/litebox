...
//! Memory-touching helpers take a `&impl VsmPlatform` and use its VTL0
...
use litebox_common_lvbs::{
    HekiKernelSymbol, HekiPatch, HekiPatchInfo, HekiRange, ModMemType, PAGE_SIZE, VsmError,
    VsmPlatform,
};
use thiserror::Error;
use x86_64::{
...
pub(crate) fn read_vtl0_contiguous<P: VsmPlatform>(
    ...
    loop {
        pages.push(
            PhysPageAddr::<PAGE_SIZE>::new(p.trunc()).ok_or(VsmError::InvalidPhysicalAddress)?,
        );
        if p == last_page {
            break;
        }
    ...
    platform.read_vtl0_bytes(&pages, offset, out)
}
...
pub struct HekiState {
    ...
    pub(crate) boot_done: AtomicBool,
    pub(crate) system_certs: once_cell::race::OnceBox<Box<[Certificate]>>,
...
impl ModuleMemoryMetadataMap {
    ...
    /// Drop a module's freed init ranges from its metadata after [`crate::mshv_vsm_free_guest_module_init`]
...
impl ModuleMemory {
    ...
    #[inline]
    pub(crate) fn write_bytes_from_heki_range<P: VsmPlatform>(
        &mut self,
        platform: &P,
    ) -> Result<(), MemoryContainerError> {
        self.text.write_bytes_from_heki_range(platform)?;
        self.init_text.write_bytes_from_heki_range(platform)?;
        self.init_rodata.write_bytes_from_heki_range(platform)?;
        Ok(())
    }
    ...
    pub(crate) fn write_bytes_from_heki_range<P: VsmPlatform>(
        ...
        for range in range {
            let phys_end = range
                ...
                .ok_or(MemoryContainerError::Overflow)?;
            self.write_vtl0_phys_bytes(platform, range.phys_addr, phys_end)?;
        }
        Ok(())
    }

    /// Write physical memory bytes from VTL0 at the specified physical address
    pub(crate) fn write_vtl0_phys_bytes<P: VsmPlatform>(
        &mut self,
        platform: &P,
        phys_start: PhysAddr,
        phys_end: PhysAddr,
        ...
        if read_vtl0_contiguous(platform, phys_start.as_u64(), &mut self.buf[old_len..]).is_err() {
