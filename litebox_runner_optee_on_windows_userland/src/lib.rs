...
/// Test OP-TEE TAs with LiteBox on unmodified Windows.
///
/// The ldelf loader and TA binaries can optionally be rewritten at runtime
/// via `--rewrite-syscalls`, or they can be pre-rewritten ahead of time.
#[derive(Parser, Debug)]
pub struct CliArgs {
    /// ldelf loader binary
    #[arg(required = true, value_hint = clap::ValueHint::ExecutablePath)]
    pub ldelf: String,
    /// Trusted Application (TA) binary
    #[arg(required = true, value_hint = clap::ValueHint::ExecutablePath)]
    pub program: String,
...
fn run_ta_with_default_commands(
    ...
    for func_id in [UteeEntryFunc::OpenSession, UteeEntryFunc::CloseSession] {
        ...
        if func_id == UteeEntryFunc::OpenSession {
            ...
            unsafe {
                litebox_platform_windows_userland::run_thread_ref(
                    entrypoints,
            ...
            unsafe {
                litebox_platform_windows_userland::reenter_thread(
                    entrypoints,
                    &mut litebox_common_linux::PtRegs::default(),
                );
            }
