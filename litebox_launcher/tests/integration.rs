...
#[test]
#[ignore] // Run with: cargo nextest run -p litebox_launcher -- --ignored test_pipe_nolibc
fn test_pipe_nolibc_end_to_end() {
    // Step 1: Compile the nolibc binary
    let test_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests");
    let c_src = test_dir.join("pipe_nolibc.c");
    let compiled = test_dir.join("pipe_nolibc");
    let status = Command::new("gcc")
        .args(["-static", "-nostdlib", "-o"])
        .arg(&compiled)
        .arg(&c_src)
    ...
    let rewritten_data = litebox_syscall_rewriter::hook_syscalls_in_elf(&elf_data, None)
        .expect("syscall rewriter should succeed");
    let rewritten = test_dir.join("pipe_nolibc.hooked");
    std::fs::write(&rewritten, &rewritten_data).expect("write rewritten binary");
    ...
    // Step 5: Run the launcher with the rewritten binary
    let child = Command::new(&launcher_bin)
        .arg(&rewritten)
        .env("LITEBOX_CENTRAL_PATH", target_dir.join("litebox_central"))
        .stdout(std::process::Stdio::piped())
    ...
    assert!(
        stdout.contains("Pipe test passed!"),
        "Expected 'Pipe test passed!' in stdout, got:\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        output.status.success(),
        "launcher exited with status: {:?}\nstdout: {stdout}\nstderr: {stderr}",
...
#[test]
#[ignore] // Run with: cargo nextest run -p litebox_launcher -- --ignored test_pipe_fork_nolibc
fn test_pipe_fork_nolibc_end_to_end() {
    // Step 1: Compile the nolibc binary
    let test_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests");
    let c_src = test_dir.join("pipe_fork_nolibc.c");
    let compiled = test_dir.join("pipe_fork_nolibc");
    let status = Command::new("gcc")
        .args(["-static", "-nostdlib", "-o"])
        .arg(&compiled)
        .arg(&c_src)
    ...
    let rewritten_data = litebox_syscall_rewriter::hook_syscalls_in_elf(&elf_data, None)
        .expect("syscall rewriter should succeed");
    let rewritten = test_dir.join("pipe_fork_nolibc.hooked");
    std::fs::write(&rewritten, &rewritten_data).expect("write rewritten binary");
    ...
    assert!(
        stdout.contains("Child received correct data!"),
        "Missing child output:\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        stdout.contains("Pipe fork test passed!"),
        "Missing parent output:\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        output.status.success(),
        "launcher exited with status: {:?}\nstdout: {stdout}\nstderr: {stderr}",
