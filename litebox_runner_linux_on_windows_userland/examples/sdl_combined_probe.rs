...
use litebox_shim_linux::host_pipe::{HostWriter, HostReader};
use std::io;
use std::sync::mpsc;
use std::time::{Duration, Instant};
use windows_sys::Win32::Foundation::{HWND, LPARAM, LRESULT, WPARAM};
use windows_sys::Win32::Graphics::Gdi::{
    BITMAPINFO, BITMAPINFOHEADER, BI_RGB, DIB_RGB_COLORS, SRCCOPY, GetDC, ReleaseDC, StretchDIBits,
};
use windows_sys::Win32::UI::WindowsAndMessaging::{
    CreateWindowExW, DefWindowProcW, DestroyWindow, DispatchMessageW,
    PeekMessageW, PostQuitMessage, RegisterClassW, ShowWindow, MSG, PM_REMOVE, SW_SHOW,
    WNDCLASSW, WM_CLOSE, WM_DESTROY, WM_QUIT, WS_OVERLAPPEDWINDOW, WS_VISIBLE,
};
...
impl io::Read for Reader {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.0.read(buf).map_err(|e| io::Error::other(format!("{e:?}")))
    }
}
struct Writer(HostWriter<WindowsUserland>);
impl io::Write for Writer {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.0.write(buf).map_err(|e| io::Error::other(format!("{e:?}")))
    }
...
fn parse_audio_frame(body: &[u8]) -> anyhow::Result<AudioPacket> {
    ...
    anyhow::ensure!(channels == EXPECTED_CHANNELS, "unsupported channels: {channels}");
    ...
    anyhow::ensure!(pcm_len % 2 == 0, "pcm length must be even: {pcm_len}");
    anyhow::ensure!(pcm_len <= MAX_PCM_BYTES, "pcm length {pcm_len} exceeds limit {MAX_PCM_BYTES}");
    ...
    unsafe {
        ...
        loop {
                ...
                Ok(rgb) => {
                    ...
                    if count < 3 {
                        if rgb[..3] != expected[count] {
                            failure = Some(format!("SDL background pixel differs: expected {:?}, got {:?}", expected[count], &rgb[..3]));
                            break;
                        }
                        ...
                        if rgb[center_pixel_offset..center_pixel_offset + 3] != [255, 255, 255] {
                            failure = Some("SDL white center rectangle not found at expected position".to_string());
                            break;
                        }
                    }
                    count += 1;
            ...
            if !last.is_empty() {
                ...
                for rgb in last.chunks_exact(3) {
...
fn main() -> anyhow::Result<()> {
    ...
    let video_thread = std::thread::spawn(move || -> Result<(), String> {
        let mut wire = litebox_desktop_transport::Framed::new(Reader(v_reader), Writer(v_writer), 1_048_576)
        ...
        while let Some(body) = wire.recv().map_err(|e| e.to_string())? {
            ...
            if received == 1 {
                wire.send(b"KEY0\x4f\x00\x01\x00").map_err(|e| e.to_string())?;
            } else if received == 2 {
                wire.send(b"KEY0\x4f\x00\x00\x00").map_err(|e| e.to_string())?;
            }
        }
        Ok(())
    });
    ...
    let audio_thread = std::thread::spawn(move || -> anyhow::Result<AudioStats> {
        let mut wire = litebox_desktop_transport::Framed::new(Reader(a_reader), Writer(a_writer), 1_048_576)
            .map_err(|e| anyhow::anyhow!("failed to create Framed transport: {e}"))?;

        eprintln!("host: audio handshake waiting");
        wire.handshake(0).map_err(|e| anyhow::anyhow!("handshake failed: {e}"))?;
        ...
        while let Some(body) = wire.recv().map_err(|e| anyhow::anyhow!("recv error: {e}"))? {
            received_count += 1;
            ...
            audio_device
                ...
                .map_err(|e| anyhow::anyhow!("WinMM playback error on buffer {received_count}: {e}"))?;
    ...
    video_thread.join().expect("video receiver panicked").map_err(anyhow::Error::msg)?;
    let frames = gui.join().expect("GUI panicked").map_err(anyhow::Error::msg)?;
    ...
    // Audio verification
    anyhow::ensure!(stats.completed >= 8, "fewer than 8 buffers completed: {}", stats.completed);
    ...
    anyhow::ensure!(stats.canceled == 0, "audio buffers canceled: {}", stats.canceled);
    ...
    println!("SDL_AUDIO_METRICS: received={} submitted={} completed={} canceled={} non_silent={} errors={} min_sample={} max_sample={} has_pos={} has_neg={} guest_exit={exit}",
        stats.received, stats.submitted, stats.completed, stats.canceled, stats.non_silent,
        stats.errors, stats.min_sample, stats.max_sample, stats.has_positive, stats.has_negative
    );
    println!("SDL_AUDIO_OK: {} verified buffers played via WinMM; guest exit 0", stats.completed);
    println!("SDL_COMBINED_OK: Video, input, and audio streams completed concurrently with clean shutdown.");
