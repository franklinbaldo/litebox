...
unsafe extern "system" fn vectored_exception_handler(
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        if exception_record.ExceptionCode == Win32_Foundation::EXCEPTION_ACCESS_VIOLATION {
            ...
            // Committed but PAGE_NOACCESS — restore access for the guest.
            // This is needed because deallocate_pages uses MEM_DECOMMIT
            // (which puts pages to MEM_RESERVE), and the gap mprotect uses
            // VirtualProtect(PAGE_NOACCESS). The guest dynamic linker may
            // access these pages during relocation. On Linux, these would be
            // handled by the kernel transparently.
            //
            // TODO: This auto-upgrade should be restricted to pages that are
            // NOT intentional guard pages (e.g., malloc arena guards). For
            // now, always upgrade to avoid crashing during library loading.
            if ok
...
exception_callback:
    // Handle the exception. The VEH set Pc=here, Sp=host_sp, X29=host_bp,
    // X1=exception_record_ptr. But ALL OTHER registers have guest values
    // from the captured CONTEXT. We must restore callee-saved registers
    // from the host stack before calling any Rust code.
    //
    // Host stack layout (pushed by run_thread_arch prologue):
    //   [sp]      = thread_ctx (str x0, [sp, #-16]!)
    //   [sp+16]   = d14, d15
    //   [sp+32]   = d12, d13
    //   [sp+48]   = d10, d11
    //   [sp+64]   = d8, d9
    ...
    ldp     x19, x20, [sp, #144]
    ldr     x0, [sp]                   // thread_ctx
    // x1 = exception_record_ptr (set by VEH in CONTEXT)
    bl      {exception_handler}
    b       .Ldone_arm64

interrupt_callback:
    // Same issue: restore callee-saved registers before calling Rust.
    ldp     d14, d15, [sp, #16]
    ldp     d12, d13, [sp, #32]
    ldp     d10, d11, [sp, #48]
    ldp     d8,  d9,  [sp, #64]
