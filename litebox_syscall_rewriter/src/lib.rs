...
fn fixup_phdr_alignment(buf: &mut [u8]) {
    ...
    let old_start = e_phoff as usize;
    let old_end = old_start + phdr_size as usize;

    // Shift forward to align: new offset is the next 8-byte boundary.
    let padding = (8 - misalignment) as usize;
    let new_start = old_start + padding;
    let new_end = new_start + phdr_size as usize;
    ...
    for i in 0..e_phnum {
        let entry_off = new_start + (i as usize) * (e_phentsize as usize);
