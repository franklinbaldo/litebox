...
//! VSM policy (module/kexec validation, HEKI patching, kernel data) resident in the runner.
...
use litebox_platform_lvbs::{
    ...
    host::{
        PRK_LEN,
        linux::{KEXEC_SEGMENT_MAX, Kimage},
    ...
    mshv::{
        ...
        vsm::{
            ...
            mshv_vsm_lock_regs, protect_physical_memory_range, unprotect_physical_memory_range,
        },
    },
};
...
/// VTL0 kernel information global, initialized on first use.
static VTL0_KERNEL_INFO: Once<Vtl0KernelInfo> = Once::new();

/// Returns the process-wide [`Vtl0KernelInfo`], initializing it on first access.
pub(crate) fn vtl0_kernel_info() -> &'static Vtl0KernelInfo {
...
pub(crate) fn vsm_dispatch(func_id: VsmFunction, params: &[u64]) -> i64 {
    let result: Result<i64, VsmError> = match func_id {
        ...
        VsmFunction::LockRegs => {
            // The end-of-boot guard was relocated here from the platform's `mshv_vsm_lock_regs`
            // because `Vtl0KernelInfo` now lives in the runner. Behavior is byte-for-byte
            // identical: same condition, same error, same error mapping below.
...
pub fn mshv_vsm_free_guest_module_init(token: i64) -> Result<i64, VsmError> {
    ...
    let freed_init_patch_targets = vtl0_kernel_info()
        .module_memory_metadata
