...
// This binary is x86-64-only: it depends on `litebox_platform_lvbs`, which is itself gated on
// `target_arch = "x86_64"` and so compiles to an empty crate elsewhere.
//
// The real entry point lives in `lvbs.rs` and is a freestanding `no_std`/`no_main` image. A
...
fn main() {
    eprintln!(
        "litebox_runner_lvbs is only supported on x86-64; this binary was built for {}.",
        std::env::consts::ARCH
    );
    std::process::exit(1);
}
