...
- **Single file** platform (1963 lines), x86_64 only (`#![cfg(target_arch = "x86_64")]`)
...
1.1. **Create `litebox/src/platform/va_partitions.rs`** with:
   - `PARTITION_SIZE: usize = 1 << 40` (1 TiB, constant).
   - `PartitionState` struct with constructor parameter for `va_max`:
     ```rust
     ...
     impl PartitionState {
         pub fn new(va_min: usize, va_max: usize) -> Self { ... }
     }
     ```
   - Use `Vec<bool>` instead of `[bool; NUM_SLOTS]` so `num_slots` can be
     computed at runtime from the platform's actual `VA_MAX`.
   - Methods: `allocate`, `deallocate`, `is_allocated`, `range_of`.

1.2. **Resolve `TASK_ADDR_MAX` TODO on Windows.** Read
     `GetSystemInfo().lpMaximumApplicationAddress` at runtime and pass to
     `PartitionState::new()`.

1.3. **Migrate Linux userland** to use the shared `PartitionState` (remove
     the duplicated `va_partitions` module from `litebox_platform_linux_userland`).
...
2.2. **`create_address_space()`** — `partitions.lock().allocate()` → slot index.
     Before marking a slot as allocated, probe the slot's entire VA range with
     `VirtualQuery` to verify no host allocations exist there. If any are found,
     skip that slot and try the next one.
...
5.1. **Add `release_memory` before `destroy_address_space` on process exit.**
     ...
     ```
     This mirrors the `execve` path (line 2016-2018).
