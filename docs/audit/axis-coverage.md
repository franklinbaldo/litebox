...
| `PB` | 46 (`PB.c2p.pie.A` … `PB.epoll_sp.nonpie.B`) | ✓ | ✓ | ✓ | ✗ | ✓ | Pipe-bridge direction is child↔parent inside one process tree; no sibling/cousin IPC topology. | Add sibling/cousin parent process with extra-pipe child to mimic VS Code ptyHost across workers. |
