...
| 5 | generic fd inheritance across fork/exec for pipes/socketpairs/stdout bridges | **COVERED** | `P.*` pipe EOF, `PB.*` pipe-bridge direction/epoll, `US6.*` socketpair, `BS.*` stdio bridge (`axis-coverage.md:72-73`, `:46`, `:68`, `:325-352`). | 🔥 | Keep; this area has had recent branch fixes (`bridge worker stdout`, `pipe nonblock`, `unix socket fork inheritance`). |
