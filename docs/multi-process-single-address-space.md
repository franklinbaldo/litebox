...
**Clone-for-fork:** `clone_for_fork()` exists on `RawDescriptorStorage` and
`FilesState` (not on `Descriptors` or `Descriptor`). The underlying
operation is `Descriptors::duplicate_raw_fd()`, which shares the underlying
...
`ProcessContext` is pure identity and lifecycle data. The wait channel
(`child_exit_notify`, a per-process condvar for `waitpid`) lives in the
internal `ProcessEntry` wrapper that `ProcessRegistry` stores in its table,
not in `ProcessContext` itself:
...
pub struct ProcessRegistry<Platform: RawSyncPrimitivesProvider> {
    table: RwLock<Platform, HashMap<ProcessId, ProcessEntry>>,
    next_pid: AtomicU32,
}
...
impl<Platform: RawSyncPrimitivesProvider> ProcessRegistry<Platform> {
    ...
    pub fn with_context<R>(&self, id: ProcessId, f: impl FnOnce(&ProcessContext) -> R) -> Option<R>;
    pub fn get_children(&self, id: ProcessId) -> Vec<ProcessId>;
    pub fn process_count(&self) -> usize;
    pub fn get_pgid(&self, id: ProcessId) -> Option<ProcessGroupId>;
    pub fn get_sid(&self, id: ProcessId) -> Option<SessionId>;
    pub fn setsid(&self, id: ProcessId) -> Result<SessionId, ...>;
    pub fn set_pgid(&self, id: ProcessId, pgid: ProcessGroupId) -> Result<(), ...>;
}
```

**Notes:**
- `create_process` takes an `exit_signal` parameter (typically `SIGCHLD`
  for fork, 0 for threads) and returns `Result<ProcessId, CreateProcessError>`.
- `wait_for_child` returns `Result<WaitResult, WaitError>` where
  `WaitResult` is a named struct with `pid` and `status` fields.
- Signal routing (`send_signal`) is **not** implemented in
  `ProcessRegistry` — it is handled at the shim layer.
- Reparenting of orphaned children is performed internally inside
  `exit_process()` and is not exposed as a public method.
...
**Locking granularity:** Each `ProcessEntry` has its own condvar for
...
> **Note:** `pid == 0` (wait for any child in the same process group) is
> currently treated as `AnyChild` with a TODO to implement proper
...
pub trait AddressSpaceProvider {
    ...
    /// When `true`, the shim eagerly copies all writable pages before
    /// spawning a vfork child (eager CoW). When `false` (default), the
    /// shim uses lazy CoW via page-fault-driven snapshotting
    /// (`try_handle_cow_fault()`). This replaces `#[cfg(target_os)]`
    /// gates with a platform-driven decision.
...
- **Private futex key** = `(u64, addr)`. The discriminator is `u64` (not
  a generic `AddressSpaceId`). Userland passes `0` for all processes
  (since they share the host VA space and addresses don't overlap). Kernel
  platforms would pass the address space ID cast to `u64`.
- **Shared futex key** = backing object identity (future work, for
  `mmap(MAP_SHARED)` pages).

`FutexManager::wait()` and `FutexManager::wake()` gain a `u64`
discriminator parameter. On userland, the shim passes `0`.
...
The shim inspects `clone()` flags to determine behavior. Fork detection is
`!CLONE_THREAD && (!CLONE_VM || CLONE_VFORK)`. There are no separate
`SyscallRequest::Fork` / `SyscallRequest::Vfork` variants — both `fork()`
and `vfork()` are converted to `SyscallRequest::Clone` with appropriate
flags.
...
and execve. The shim protects parent memory using one of **two CoW strategies**,
selected by the platform's `AddressSpaceProvider::EAGER_COW_FOR_VFORK` constant:

- **(a) Lazy CoW** (default, `EAGER_COW_FOR_VFORK = false`): Before spawning the
  child, the shim marks all writable guest pages read-only. On the first write
  fault, `try_handle_cow_fault()` snapshots the faulting page, restores write
  permissions, and records it as dirty. After the child signals vfork_done, dirty
  pages are restored from snapshots.

- **(b) Eager CoW** (`EAGER_COW_FOR_VFORK = true`): Before spawning the child,
  the shim eagerly copies all writable guest pages. After the child signals
  vfork_done, all writable pages are restored from the copies.

Both strategies use the same `CowState` struct, which tracks `protected_ranges`
(the set of VA ranges under CoW protection) and `dirty_pages` (pages that have
been snapshotted). The snapshot must be set up *before* `spawn_thread` to capture
clean, pre-modification state.
...
- Implement `clone_for_fork()` on `RawDescriptorStorage` and
  `FilesState<FS>` — sharing underlying `Arc` file objects (POSIX
  shared-file-description semantics) via `Descriptors::duplicate_raw_fd()`
  while copying the FD-number mapping and `close_on_exec` flags.
...
- Introduce `ProcessState` struct holding per-process `PageManager` (`pm`)
  **and** `AddressSpaceId` (`address_space_id`). Threads of the same
  process share `Arc<ProcessState>`.
...
- **Deferred note from Step 1.4:** Moving `FilesState`, signal handlers,
  ...
  `files_struct*`, `sighand_struct*`, `cred*`). Task fields like
  `FilesState` are wrapped in `RefCell<Arc<...>>` to support mutation on
  `execve()` (e.g., closing `O_CLOEXEC` FDs requires replacing the inner
  `Arc`). `ProcessState` stays lean — `PageManager` and
  `AddressSpaceId` (analogous to `mm_struct`), the structures that are
...
- Add a `u64` discriminator parameter to `FutexManager::wait()` and `wake()`.
- On userland: pass `0` (addresses don't overlap between processes).
- On kernel: pass the process's `AddressSpaceId` cast to `u64` to prevent
  false aliasing.
...
| **litebox_common_linux** | Add `Wait4`, `Execve` variants to `SyscallRequest` (if not already present). Fork and vfork are converted to `SyscallRequest::Clone` with appropriate flags — no separate `Fork`/`Vfork` variants. |
