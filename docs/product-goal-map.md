...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro, 5/5 native PASS + 5/5 litebox FAIL); `copilot::pminus.*` 4/22 under litebox; 5 deterministic minimal PTY probes (3 shim divergences captured + fixed; 2 confirming probes prove the broker dup'd-slave data path works in simpler shapes) | **Regression window**: `708fd18..33a2453e`. TUI mode itself does not function under litebox. Surfaced by Goal A validation session 7c1fc95d against amalgamation `898af46a`. **Three shim divergences identified and fixed this session**: (a) `TIOCGPTN` on a PTY slave fd returned `pty_id()` instead of `-ENOTTY` (probe `076216a5`, fix `7858a614`); (b) `openat("/dev/pts/", O_DIRECTORY)` returned `ENOENT` instead of falling through to rootfs FS (probe `8d5218b4`, fix `8c373d50`); (c) `readlink("/proc/self/fd/{0,1,2}")` returned generic `/dev/std{in,out,err}` instead of `/dev/pts/<pty_id>` for `BrokerPty` slaves (probe `a725f425`, fix `b65b6047`). All five PTY probes (3 divergence probes + 2 confirming probes) PASS native AND litebox. **However, `copilot::tui_noLLM.startup_then_exit` still hangs identically.** Post-fix audit log shows copilot successfully opens `/dev/pts/1` and writes 78+166+81 bytes of TUI escapes — but those writes never reach the harness PTY master. **Fourth divergence localized to dropbear's session-handler vfork→close-master lifecycle under litebox**: worker 27 (dropbear connection handler) opens `/dev/ptmx` as fd 6 at seq 404, vforks the user shell at seq 455 (clone flags 0x4100 = CLONE_VM|CLONE_VFORK), then closes its master at seq 457 — leaving the master fd held only by the user-shell process tree (bash → copilot → node, workers 44/72/98). The dropbear connection handler then does auth/utmp/log work and exits at seq 576 without ever running a master-read forwarding loop. The vfork'd user-shell tree inherits the master but neither bash nor copilot read from it (they expect SOMEONE to forward master→SSH). Native dropbear must have a different architecture for this — either a separate forwarder process or a fork pattern that keeps the parent alive in a master-read loop. The bug appears to be **in dropbear's adaptation to litebox's vfork semantics**, not in any individual shim syscall; the fix surface is likely: (i) audit litebox's `clone(CLONE_VFORK)` shim semantics vs native Linux to find behavioral divergence that broke dropbear's assumption, OR (ii) (less likely) carry a small dropbear patch. This is a deeper investigation than a single shim function; explicit handoff. **Blocks** the W8/W10 row below: `find_head` pipeline behavior can't be validated until TUI mode itself starts. |
