...
flowchart TD
    ...
    UDS_NAMED_STREAM[Named UDS stream broker-held: bind/listen/accept + in-band SCM + fork-survive]:::landed
    ...
    %% extension host steady-state (high-concurrency vfork-quiesce + FS)
    EH_STEADY[Extension Host steady-state: vfork-quiesce under parked libuv threadpool + high-thread-count FS open]:::landed
    FORK_PIPE --> EH_STEADY
    EH_STEADY --> GB

    %% extension host heap headroom (ELF interpreter top-down so node's brk isn't capped at PIE_LOAD_OFFSET)
    EH_HEAP[Extension Host heap headroom: ELF interpreter loaded top-down so dynamically-linked ET_EXEC node brk grows past PIE_LOAD_OFFSET]:::landed
    CLI_NODE --> EH_HEAP
    EH_HEAP --> GB

    %% local-epoll EPOLLET edge accounting (agent-host reactor busy-spin)
    EPOLL_ET[Local epoll EPOLLET edge-dedup on sticky-OUT broker inet sockets -- agent-host tokio reactor spin]:::landed
    BROKER_INET --> EPOLL_ET
    EPOLL_ET --> GB
...
| **Extension Host steady-state: vfork-quiesce under parked libuv threadpool + high-thread-count FS open** | ✅ | `vscode::extension_host_spawn` (node spawn-storm: `THREADPOOL_SIZE=96`, 60 `posix_spawn`s, 180s budget — wedges pre-fix at pool≥64, passes post-fix); `MTF.{epoll_eventfd,futex_cond,mixed}.{fork_exit,fork_exec,posix_spawn,posix_spawn_uds}.{dpg,dng,spg}` multithread-fork repro (branch `wportnoy/mt-fork-repro`; genuine-deadlock config `MTF.futex_cond.posix_spawn.dng`) | Session `05a42e59`. VS Code's **Extension Host reconnect loop** was two independent deadlocks that only manifest under VS Code's high-concurrency node process (libuv threadpool + V8 workers) and are masked at low thread counts. **(1) vfork-quiesce lost-wakeup** (commit `b9ec523b`): a NonPieGlibc node `posix_spawn` (glibc → `clone(CLONE_VM\|CLONE_VFORK)`) must quiesce sibling threads, but `ThreadHandle::interrupt` was *edge-triggered* — a libuv sibling entering guest `FUTEX_WAIT` just after the one-shot interrupt lost the wake, never parked, so `parked_count` never reached `expected` and the forker spun in `clone()` forever. Fix re-arms `thread.interrupt()` per still-suspended sibling each iteration in `park_other_threads` (`litebox_shim_linux/src/syscalls/process.rs`) and makes the sibling park checkpoint `park_for_vfork_if_requested` (`litebox_shim_linux/src/wait.rs`) treat `is_suspended` *level-triggered* + spin until the park is published. **(2) FS layered-cache RwLock self-deadlock** (commits `ef121ae2` + correction `f080b1b3`): `litebox/src/fs/layered.rs::open()`'s cache-hit fast path held `self.root.read()` across a 9P `fstat` (`lower_fd_is_shareable`) *and* an inner `self.root.read()`; the writer-preferring `RwLock` self-deadlocks (state `0xc0000001`) when a writer queues between the outer and inner read — only reachable at pool≥48, so real VS Code (default `UV_THREADPOOL_SIZE=4`) and the low-thread inherit tests never hit it. `ef121ae2` released the outer read but re-read `lower_access_modes` *fresh* against a stale snapshotted entry (TOCTOU) → wrong fid-compat decision → regressed `vscode::bootstrap`; `f080b1b3` snapshots `(entry, access_modes)` together under one read, releasing before the body. Validated: node pool=128 3/3 complete, `vscode::` suite 12/12, FS+fork regression sample 323 pass / native 0-FAIL, dashboard 0 hard regressions. **Loosens the prior bootstrap→listen→TCP-connect stopping point** — `extension_host_spawn` exercises the steady-state extension-host spawn storm, the layer where the reconnect loop actually lived. |
| **Extension Host heap headroom: ELF interpreter loaded top-down so a dynamically-linked ET_EXEC node's `brk` heap isn't capped at `PIE_LOAD_OFFSET`** | ✅ | `HEAP.brk_past_interp.nonpie-glibc` (a non-PIE-glibc leaf grows `brk` by +512 MiB via the raw `brk` syscall and asserts it crosses the old 256 MiB ceiling; **fails pre-fix** `got=-1` with `ld-linux` mapped at `0x10010000` = `VA_MIN + PIE_LOAD_OFFSET`, **passes post-fix**; native passes both) | Session `05a42e59`, commit `9921eb6f`. VS Code's bundled `node` is a dynamically-linked **ET_EXEC** (non-PIE) loaded at its canonical low vaddrs; its glibc `brk` heap grows upward from there. The ELF interpreter (`ld-linux`) was reserved at the fixed `PIE_LOAD_OFFSET` (`addr_min + 256 MiB`), sitting directly above the main image and **capping `brk` growth** — the extension host OOM-crashed the instant its heap needed to cross 256 MiB (`brk` → `ENOMEM` → self-`SIGSEGV` via `tgkill`), surfacing as the "Remote Extension host terminated unexpectedly 3 times within 5 minutes" reconnect churn. Fix (`litebox_shim_linux/src/loader/elf.rs`): new `ElfFile::load_high` makes the interpreter's `reserve()` hint at `addr_max()` so `sys_mmap` falls through to its top-down search — mirroring how the kernel places `ld.so` for an ET_EXEC main and leaving the whole gap above the main image free for the heap. **No-op for PIE mains** — their interpreter already loaded top-down (its `PIE_LOAD_OFFSET` hint overlapped the PIE main and fell through to the same search), so only the ET_EXEC case changes; ET_EXEC workers get the full base-0 address space (`run_worker_exec`), so the interpreter lands at exactly `VA_MIN + PIE_LOAD_OFFSET` pre-fix. **Companion fix in the same merge** (`9937d3d5`): `getsockopt(SOL_SOCKET, SO_TYPE)` is now answered locally from `SocketIoClass` for SCM-passed broker sockets — libuv's `uv_guess_handle` was getting `EOPNOTSUPP` → `UV_UNKNOWN_HANDLE` and node refused to adopt the exthost's passed TCP socket (guarded by `assert_so_type` in `scm_rights.rs`). Validation: end-to-end on docker the extension-host node grew to **665 MB RSS and stayed alive 2.4 h** (pre-fix OOM-crashed at 256 MB), whole-session audit **0 `brk` ENOMEM / 0 self-SIGSEGV**; dashboard full coverage (litebox 5801/5801) **0 hard regressions**. |
...
| Test harness: broker ELF-cache pre-warm covers all exec'd coreutils + deps | ✅ | wide cold sweep across CF/BASH/XM/XB/SC/S/PROC/NA = 0 runtime rewrites (residual fails are SCM/XW3 load flakes) | Session `05a42e59`, commit `1701dec5`. The broker pre-warm list (`litebox_broker/src/main.rs`) only covered bash/cat/echo/grep/tr/true/find/head/stty/node, so fork/exec tests exec'ing other coreutils rewrote them at runtime — tripping the harness `unexpected runtime ELF rewrites` assertion on each test's *cold first run*. This was the dominant rotating "hard regression" noise on fix-branch dashboard shadows (8-10 distinct warm-passing tests/gate: dd/wc/chmod/env/readlink/sed/xargs/ls/mkdir/hostname/uname). Added the missing coreutils + their ldd deps (libacl, libcrypto) + nss/resolver libs; all in-image so mtime is stable, ELF cache hits on first 9P read. |
| Lock re-entrancy safety: compile-time prevention on descriptor_table + runtime detector | ✅ | `sync::rwlock::tests::lock_tracing_{panics_on_same_thread_reentrant_read,panics_on_write_while_holding_read,catches_nested_descriptor_table_read_repro,allows_same_thread_reads_of_different_rwlocks}` (4/4); `wportnoy/lock-safety-session` dashboard 5800/5801 litebox + 5801/5801 native, 0 hard regressions | Session `05a42e59`. Two complementary efforts against the re-entrant writer-preferring-RwLock self-deadlock class (`a863a31f` snapshot_fd_table, `f080b1b3` FS layered-cache). **(C) compile-time prevention**: `fd_path`/`descriptor_path` take a passed-in `&Descriptors` view and the lookup helper is `&self`-less, so it cannot re-acquire the global `descriptor_table()` RwLock — re-entrancy on that path is now a compile error; also retires the `a863a31f` deferred-lookup workaround (snapshot_fd_table passes its held `&*dt` directly). **(B) runtime detector**: the `lock_tracing` feature gains a same-thread re-entrant-lock PANIC naming the lock creation-site + acquisition-site (covers read→read and any-held→write; not write→read), plus a CI lane `cargo test -p litebox --features lock_tracing sync::rwlock::tests::`. Detector currently instruments `RwLock` only. Follow-ups: extend detector to `Mutex` (net/nine_p use it); thread `&Descriptors` to detector-confirmed re-entrant sites only (avoid blanket FS-trait conversion of ~15 methods / 546 call sites). **Mutex coverage + hardening + first real fix (`wportnoy/lock-tracing-mutex`, cleared at ~89% dashboard coverage, 0 hard regressions)**: detector extended to `Mutex` (`sync::mutex::tests::*`) and made **per-thread** via a `ThreadIdentityProvider` (Linux `gettid`, Windows `GetCurrentThreadId`, mock TLS ids) so it no longer misreports cross-thread *contention* as re-entrancy — this collapsed a discovery run from 161 noisy logs to **2 genuine same-thread sites**; the harness gains an `LITEBOX_LOCK_TRACING` runner-feature passthrough so the detector can run inside integration trials. The hardened detector surfaced + confirmed one dominant real site (96×): `with_socket` held a `raw_descriptor_store` read guard across its `.map_err` error path (`assert_no_unhandled_socket_subsystem` re-reads it) — **fixed** by resolving the fd in an inner scope so the guard drops before the error path. **Methodology lesson**: detector-driven discovery (harden → re-run → fix surviving sites) beats blanket conversion; the remaining `descriptor_table` read-while-read site (~6-8×) is a documented fast-follow. **`worker_exec_host_stdio_source_fd` re-entrant read fixed (`507ee3c6`, session `05a42e59`, landed via `wportnoy/exec-stdio-reentrancy`)**: the exec-stdio bindings held a `descriptor_table()` read across the `.or_else` fallback chain whose first fallback (`worker_exec_tty_stdio_source_fd`) re-reads it — a live latent site the dashboard masked as a ~30% exec flake. Surfaced deterministically by the `LITEBOX_LOCK_TRACING` integration sweep (14/234 `FS.{exec,open}_*_fs-exec.txt` panic pre-fix at `litebox.rs:125`, **0 post-fix**; matched amalgamation-tip control clean). Fix drops the guard before the fallbacks. Broad `FS`+`CL3` (234) sweep on the fix now clean; the 26 dashboard 'hard_regression' entries on the branch were confirmed flakes (98/98 pass in isolation). **Architectural note**: the landed `fd_path`/`descriptor_path`/`with_socket` `&Descriptors`-threading is *plumbing*, not enforcement — the trait methods retain `&self`, so `self.descriptor_table()` re-acquisition still compiles (only the innermost `&self`-less lookup helper is statically prevented). True end-to-end static prevention requires *sealing* the `descriptor_table()` accessor so the threaded view is the only access path (planned follow-up); until then the `LITEBOX_LOCK_TRACING` integration sweep is the completeness oracle. **FS-layer seal landed (Phase 1, session `05a42e59`, `wportnoy/seal-descriptor-table`)**: the `LiteBox` field is removed from all 5 FS backends (`in_mem`/`layered`/`tar_ro`/`nine_p`/`devices`), so `self.litebox.descriptor_table()` re-acquisition is now a **compile error** — the FS layer is statically sealed (delivering the "statically impossible" goal there). 9 path-based trait methods (`file_status`/`read_link`/`unlink`/`rename`/`mkdir`/`rmdir`/`symlink`/`chmod`/`chown`) now take `&[mut] Descriptors`; 34 shim callers acquire-at-entry and thread down; the layered `file_status` recursion re-acquire bug is fixed **by construction** (compiler-forced). This **retroactively enforces the landed `fd_path`/`descriptor_path`** (FS trait methods on now-fieldless backends). Also fixed a `worker_exec_stdio_is_unsupported` shim re-entrant read that the threading base bound as a live guard across `worker_exec_host_stdio_source_fd` (scoped the guard; `f0fb21f3` was clean, isolation-confirmed). Validated: 0 production `descriptor_table` acquires in `fs/` (grep); `LITEBOX_LOCK_TRACING` FS+CL3 sweep **0 reentrancy**; dashboard gate 5786/5801 litebox **0 hard regressions** (`FS.bg-open-read`/`CL3.*` fails confirmed **pre-existing** load flakes via matched-N vs baseline — baseline flakes `FS.bg-open-read` 4× vs seal 3× under identical load). **Remaining phases**: Phase 2 = seal `raw_descriptor_store` (`with_socket`, `net.rs` — a *separate* RwLock, same field-removal rule); Phase 3 = shim `Task` entry/inner split for the remaining shim-internal `descriptor_table` nesting (the shim is the legit top-level acquirer, so full static sealing there needs a bigger restructure). |
...
| Broker virtual DNS resolver: CNAME-heavy UDP truncation + TCP fallback | ✅ | `BL.dns_resolve_cname_heavy.pie-glibc.dpg1`; `vscode::*` 8/8 | Work stream `wportnoy/broker-dns-cname` fixed VS Code Remote Server startup hangs on Microsoft/Azure Traffic Manager hostnames. Root cause: upstream UDP DNS replies for CNAME-heavy names set TC=1; glibc then opened TCP to the virtual resolver (`10.0.0.1:53`), but only UDP was special-cased. The broker now maps TCP connects to the virtual DNS address onto the discovered host DNS server, preserving normal libc/c-ares DNS-over-TCP fallback. The temporary demo `--add-host` workaround was removed. |
...
| Broker-held timerfd | ✅ | `TFD.*` (same-process arm/read/poll), `INHERIT.timerfd.*` (fork inheritance, 5×5 binary cross-product) | This session (work stream `wsT`, consolidated on `wportnoy/fd-tax-all`). The broker owns the armed timer + a firing thread; the worker holds a token, so timerfd survives fork/exec without snapshot/restore. New: `litebox_broker/src/cwfd/timerfd_state.rs`, `broker_timerfd_provider.rs` (shim + runner). Two follow-on fixes: broker-router timerfd opcode routing (`742ecb2f` — opcodes were hitting the `other =>` arm → connection close → 134 `TFD`/`INHERIT.timerfd` FAILs), and pre-subscribe-at-creation so blocking `read()` wakes (not just poll/epoll) (`4d9375a1`, `TFD.basic_arm_and_read`). Per the cross-boundary principle this is a defensible broker-held resource (crosses fork/exec with live timing state); the one cost is per-op IPC for timer-hot workloads. |
| Socket-I/O syscall exhaustiveness + `FdKind` single-taxonomy + eager-unconditional broker sockets | ✅ | `BL.dns_resolve_cname_heavy.{static-pie-musl,non-pie-static-musl}.*` (musl `recvmsg` DNS-over-TCP), `UDS_{STREAM,SEQPACKET,DGRAM_SCM}.*`, `SCM.*`, `SK.subtree.{direct,deep,exit_then_kill}` | This session, consolidated on `wportnoy/fd-tax-all` (dashboard: native clean, litebox `fixed=12`, zero regressions vs `0fef3736`). (1) **Socket-I/O gate**: every socket syscall handler (`do_sendto`/`do_sendmsg`/`do_recvfrom_with_fds`/`sys_recvmsg`/connect/bind/listen/accept/shutdown/get+setsockopt) routes through one exhaustive `socket_io_class(&RawFdRef)` match (rustc E0004 + `wildcard_enum_match_arm = "deny"`), replacing per-handler `try_with_broker_*` probe chains — closes the latent gap where `do_sendmsg` lacked the `tcp_conn` arm `do_sendto` had (musl's DNS-over-TCP `sendmsg` → `ENOTSOCK` hung the VS Code agent-host). Broker request-router + `state_service` also made exhaustive over `Opcode`. (2) **`FdKind` collapse**: snapshot/fork fd taxonomy collapsed to one `FdKind` (survey `docs/fd-taxonomy-survey.md`); `FdClass`/`BrokerHandleSnapshot`/`ReplacedSubsystem` deleted. (3) **Eager-unconditional**: removed the `EAGER_BROKER_SOCKETPAIR_ENABLED` runtime gate + env forwarding — AF_UNIX socketpair/dgram/seqpacket are broker-backed unconditionally (refines the `=1 default` note above). (4) **Regression fixes folded in**: `UDS_DGRAM_SCM.file_across_dgram` (broker unix-DGRAM now tokenizes arbitrary file fds via `SubsystemTag::File` + layered-FS fid delegation; was `EOPNOTSUPP`) and `SK.subtree.*` (the SIGKILL local fast path now stamps broker pid-exit before notifying waiters, so broker-held pidfds observe a SIGKILL'd process's exit). Named AF_UNIX **stream** sockets are now broker-held via the native `BrokerUnixStream` subsystem — see the row below (VS Code `code-server --socket-path`). |
| Named AF_UNIX **stream** sockets broker-held (`BrokerUnixStream`: bind→listen→accept, in-band SCM_RIGHTS, fork/migration survival) | ✅ | `U.server_fork.{A,AA}.{pie-glibc,nonpie-glibc,static-pie-glibc,static-pie-musl,non-pie-static-musl}.child_connect` (forked child connects to a parent's named listener + echo round-trip, 24/24 incl. native baseline), `US{1..6}`/`UA.*`/`UDS_STREAM.*`/`SCM.*`-over-named-stream; broker units 329/329 | Work stream `wportnoy/uds-named-stream` (live-dashboard validated, 0 genuine regressions). Replaces the local `UnixSocketSubsystem` cross-worker TCP+sidecar bridge with a first-class native broker subsystem (`unix_stream_state.rs` global addr table + accept backlog + byte-stream peer wiring; 10 opcodes + wire DTOs; common+runner providers; shim subsystem + gate cascade; `socket(AF_UNIX,SOCK_STREAM)` routing). Bundles seqpacket SCM_RIGHTS token transport, opcode-wildcard exhaustiveness hardening (`Opcode::TryFrom<u8>` / `response_for` / the conn creator-tracking match — the latter reframed in AGENTS.md as the cautionary wildcard example), and phase-5 deletion of the dead local impl (−735 LOC). **Fork subscription bug fixed** (`d3220d1e`): a broker-backed fd object and its single broker subscription are `Arc`-shared across forked fd-table slots; the per-slot `on_close` unconditionally `force_unsubscribe`d, so a forked child's CLOEXEC close of an inherited named-listener fd tore down the **parent accept loop's** subscription (deterministic `U.server_fork.child_connect` read-timeout, nearly mis-dismissed as a load-flake). Generalized PTY's `slot_count` into `BrokerBackedCommon` (`slot_refs` + `note_slot_dup` + `force_unsubscribe_if_last_slot`) across the 8 broker-socket/inotify subsystems — unsubscribe only on the last slot close — which also closes the same latent hole in `tcp_conn`/`seqpacket`. |
| **Local epoll EPOLLET edge accounting on sticky-OUT broker inet sockets** | ✅ | `EP.spin.broker_tcp_sticky_out_et.{dpg1,dpg1_dng_spm,vscode_cli,vscode_node}` (spin repro: ~1300–2700 local epoll iterations pre-fix vs ~30 native/post-fix), `EP.spin.broker_udp_writable_et.*` + `EP.spin.{eventfd_idle,timerfd,timerfd_inherited_fork}.*` (idle negative controls — must NOT spin), `EP.`/`EPI.`/`POLL.` 194/194 | Session `05a42e59` (`wportnoy/epoll-edge-fix`, repro `f4bba6f4` + fix `f0d73de4`, merged `--no-ff` onto fd-tax `c38e00e2`). After the DNS-over-TCP gate (row above) unblocked name resolution, the agent-host (`cli-alpine-x64`, StaticPieMusl) tokio reactor **busy-spun** instead of starting the workbench server, because litebox's worker-local epoll treated every broker-fd notification as a fresh EPOLLET edge. `BrokerInetDgram::current_events` is unconditionally writable and `BrokerTcpConn` re-emits sticky `OUT` after every read/write/shutdown, so an EPOLLET interest watching `OUT` received a spurious edge per notification → reactor feedback-spin (millions of `epoll_pwait`+`clock_gettime`). Fix tracks per-interest `last_delivered_events` and only delivers an EPOLLET event when a watched bit actually transitions, scoped by an exhaustive `DescriptorRef::edge_dedup_mask()` — `Events::OUT` only for `BrokerInetDgram`/`BrokerTcpConn`, `Events::empty()` for every other fd kind, so it is a byte-for-byte no-op for PTY/pipe/pidfd/eventfd/signalfd/inotify (both mutation sites gated on `!edge_dedup_mask.is_empty()`). Epoll stays worker-local by design (it must watch worker-local fds; a broker-held epoll cannot). **Validated clear-to-land**: 0 hard/soft regressions on the dashboard; the 23 low-confidence `new_fail` were pre-existing flakes (real-LLM `copilot::tui.llm.*` + PTY `dropbear_bash.*` + `RL.pidfd`) that fail at the same rate on the amalgamation itself and unrelated branches (proven by cross-branch verdict history + an isolated serial re-run), and the fix is a provable no-op for all of them. |
