...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro); 6 minimal PTY+signal probes (4 shim divergences fixed; 2 confirming) | **Regression window**: `708fd18..33a2453e`. **Four shim divergences identified and fixed by session 7c1fc95d**: (1) TIOCGPTN-on-slave returned pty_id instead of ENOTTY (commit `7858a614`); (2) openat("/dev/pts/", O_DIRECTORY) returned ENOENT instead of falling through to rootfs FS (`8c373d50`); (3) readlink on /proc/self/fd/{0,1,2} returned generic /dev/std{in,out,err} instead of /dev/pts/<pty_id> for BrokerPty slaves (`b65b6047`); (4) SIG_DFL for Stop-disposition signals (SIGTSTP/SIGTTIN/SIGTTOU/SIGSTOP) terminated the process instead of being a no-op — Native default action is STOP; pre-fix shim treated Stop as Terminate, killing apps that do job-control patterns like Copilot's launcher self-kill (`27393b02`). All probes PASS native + litebox. **Fourth divergence diagnostic chain**: wait4 status logging revealed `status_raw=149` (= 128 + 21 = SIGTTIN kill encoding) on both copilot-linux-x64 (pid 15) and its node child (pid 26). Bisecting signal-delivery sources (broker pgrp-signal callback logging) ruled out the broker pty input-character path. Tracing remained shim-internal: deliver_signal in `litebox_shim_linux/src/syscalls/signal/mod.rs` at the SIG_DFL Stop-disposition arm. **Post-fix state**: copilot no longer dies of SIGTTIN. The launcher and node child stay alive (worker 98 = node child now has 179K syscalls vs 34K pre-fix). But `copilot::tui_noLLM.startup_then_exit` still hangs: copilot now sends 244+ bytes of TUI capability queries (OSC 10/11/4 color queries) and waits for emulator responses. The harness DOES send those responses via `emu.take_pty_writes()`; whether the response timing/format matches what copilot expects is the next investigation layer — **NOT a shim function bug**, but a terminal-protocol-negotiation interaction. Likely surfaces: emulator response latency under SSH+broker buffering, ANSI parser quirks in copilot's specific terminal-detection logic, or copilot-specific timeout assumptions about reply arrival. **Blocks** the W8/W10 row below. |
