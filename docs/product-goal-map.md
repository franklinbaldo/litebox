...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro); 5 minimal PTY probes (3 shim divergences fixed; 2 confirming) | **Regression window**: `708fd18..33a2453e`. **Three shim divergences identified and fixed**: TIOCGPTN-on-slave (commit `7858a614`), /dev/pts/ openable (`8c373d50`), readlink-on-stdio-returns-BrokerPty-path (`b65b6047`). All five PTY probes PASS native + litebox. **Fourth divergence: copilot's own internal exit cascade, not a shim bug.** Architecture (extracted from npm shim source `/usr/local/bin/copilot`): the shim is a tiny node script that does `spawnSync('@github/copilot-linux-x64/copilot', argv, {stdio:'inherit'})` then `process.exit(o.status ?? 1)`. The 160MB native binary copilot-linux-x64 (worker 72 in audit) IS the TUI process; it spawns its own node child (worker 98 = guest pid 26) for the UI. **Audit + native-strace diff completed**: native dropbear connection-handler reads ~50 times from its pty controlling endpoint and forwards via SSH; litebox dropbear worker 27 does THE SAME (reads 78+166+81 payload bytes, writes ~556 SSH-framed bytes via writev fd=0). **Bytes DO reach the harness** — 88 bytes of TUI teardown escapes confirmed in transcript: kitty-keyboard-pop, alt-screen toggle-off, mouse-mode-disable, cursor restore. **Cascade**: worker 98 (node child) is interrupted in `read(fd=20)` (audit ends with EINTR, no exit_group recorded). ~86 ms later, worker 72 (copilot-linux-x64) `wait4(26, WNOHANG)` returns 26 — per litebox process registry source, wait_for_child only reports state change for terminated zombies, so pid 26 DID exit. Worker 72 then writes 81 bytes of TUI teardown, kill(15, SIGTTIN) on itself, exits. Worker 44 (npm shim) wait4(15) returns 15; shim does `process.exit(o.status ?? 1)` = exit 1. SSH session closes. **What's still unknown**: the signal that interrupted node's read. No `kill`/`tgkill` syscall in the audit targets pid 26; signal must be from kernel-level mechanism (controlling-tty SIGHUP, pgrp signal, broker-injected). Verified directly via SSH: bash works, `copilot --version` works (the npm shim handles `--version` differently), only TUI mode triggers this cascade. **Resolution requires**: (a) reading the bundled copilot-linux-x64 (Go or Rust native binary) — disassembly to find what triggers the wait4 / pid-26 monitoring; (b) gdb-debugging node-under-litebox to find when/why fd 20 returns EINTR; (c) examining litebox's signal delivery to see what signal node received. All are surfaces deep beyond shim function fixes. **Blocks** the W8/W10 row below — `find_head` can't be validated until TUI starts. |
