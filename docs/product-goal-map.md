...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro, 5/5 native PASS + 5/5 litebox FAIL); `copilot::pminus.*` 4/22 under litebox; `PTY.ioctl.tiocgptn_only_on_master.pie-glibc.dpg1` + `PTY.devpts.dir_openable.pie-glibc.dpg1` (deterministic minimal probes — two shim divergences each captured and fixed) | **Regression window**: `708fd18..33a2453e`. Even baseline `tui.simple_math` / `tui.simple_bash` fail — TUI mode itself does not function under litebox independent of any shell-tool pipeline. Surfaced by Goal A validation session 7c1fc95d against amalgamation `898af46a` (`.dashboard/results.sqlite` run_id 30281). **Two shim divergences identified and fixed this session**: (a) `TIOCGPTN` on a PTY slave fd returned `pty_id()` instead of `-ENOTTY` (probe `076216a5`, fix `7858a614`); (b) `openat("/dev/pts/", O_DIRECTORY)` returned `ENOENT` instead of falling through to the rootfs FS (probe `8d5218b4`, fix `8c373d50`). Both minimal probes now PASS native and PASS litebox. **However, `copilot::tui_noLLM.startup_then_exit` still hangs identically post both fixes**: audit log shows node's main thread polling in libuv's epoll loop (fd 13, ~190ms ticks via internal eventfd on fd 16) and consuming wake-ups, but **never writing to fd 1 (stdout)** — so no TUI ever renders. All the syscalls node makes now succeed under litebox; the divergence is no longer at the syscall-errno layer. The next investigation needs a native-side comparison (strace against the same SSH+dropbear+PTY shape) to find what trigger drives native node to start rendering that litebox's node never receives — possibly a missed signal, a misrouted fd, or a libuv-internal event that doesn't fire. **Blocks** the W8/W10 row below: `find_head` pipeline behavior can't be validated until TUI mode itself starts. |
