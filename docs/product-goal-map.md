...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro, 5/5 native PASS + 5/5 litebox FAIL); `copilot::pminus.*` 4/22 under litebox; `PTY.ioctl.tiocgptn_only_on_master.pie-glibc.dpg1` (deterministic minimal probe — one shim divergence captured) | **Regression window**: `708fd18..33a2453e`. Even baseline `tui.simple_math` / `tui.simple_bash` fail — TUI mode itself does not function under litebox independent of any shell-tool pipeline. Surfaced by Goal A validation session 7c1fc95d against amalgamation `898af46a` (`.dashboard/results.sqlite` run_id 30281). **Partial progress in session 7c1fc95d**: (a) deterministic LLM-free integration repro landed (`copilot::tui_noLLM.startup_then_exit`, commit `82c4fab7`); (b) strace divergence sub-investigation identified that the shim's `TIOCGPTN` ioctl handler returned `entry.pty_id()` for any PTY entry, including slaves where native returns `-ENOTTY`; (c) minimal harness probe `PTY.ioctl.tiocgptn_only_on_master` captured the divergence (commit `076216a5`); (d) shim fix landed (commit `d1b81e5b` — added `entry.is_master()` gate). Post-fix the minimal probe passes under litebox, **but the integration-level repro `copilot::tui_noLLM.startup_then_exit` and the original `copilot::tui.*` trials still hang identically** (empty screen, kernel still cooked-mode, `/exit` echoed by line discipline). The TIOCGPTN divergence was a real shim bug worth fixing in isolation, but is not the causative root of copilot's TUI hang. **At least one more divergence remains** between native and litebox in copilot's TUI startup; another strace pass with the TIOCGPTN fix in place is needed to identify the next divergence point. **Blocks** the W8/W10 row below: `find_head` pipeline behavior can't be validated until TUI mode itself starts. |
