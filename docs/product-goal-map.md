...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro); 5 minimal PTY probes (3 shim divergences fixed; 2 confirming probes) | **Regression window**: `708fd18..33a2453e`. **Three shim divergences identified and fixed** (commits 7858a614, 8c373d50, b65b6047). All five PTY probes PASS native + litebox. **Fourth divergence root-caused (not a shim bug)** via native-strace vs litebox-audit-log diff: native dropbear's connection-handler reads ~50 times from its pty controlling endpoint and forwards to the SSH socket. Under litebox dropbear, the connection-handler (worker 27) does the same forwarding — reads 78+166+81 payload bytes and writes ~556 SSH-framed bytes via writev to fd 0. **SSH writes DO reach the harness** (confirmed: bash and `copilot --version` round-trip correctly over SSH+dropbear via the same path). The bytes forwarded back are TUI-mode TEARDOWN escape sequences (alt-screen toggle, mouse-mode disable, cursor restore) — copilot enters TUI mode, immediately tears down WITHOUT rendering the trust modal. Audit of copilot's sub-workers shows the launcher waits for its node child at seq 2617 wait4(26), child state changes, launcher concludes session should end (checks `/root/.copilot/restart/<pid>.json` + `/root/.copilot/crash-context/<pid>.json` — both ENOENT, normal end-of-life checks). The fourth "divergence" is therefore **copilot's own internal exit-decision logic** triggered by some env/IPC signal between its node sub-workers under litebox — not another shim function. Resolution requires: (a) reading copilot's bundled .js to find what monitors the node child's state and triggers cleanup, OR (b) gdb-debugging node-under-litebox to find the actual decision point, OR (c) extending the integration setup to log copilot's own diagnostics (`/root/.copilot/logs/`). This is genuine escalation territory — outside "fix a shim function". **Blocks** the W8/W10 row below: `find_head` pipeline can't be validated until TUI mode renders. |
