...
| **TUI mode startup + interactive rendering (Copilot CLI)** | 🔴 | `copilot::tui.*` 0/11 under litebox vs 11/11 native; `copilot::tui_noLLM.startup_then_exit` (deterministic LLM-free integration repro, 5/5 native PASS + 5/5 litebox FAIL); `copilot::pminus.*` 4/22 under litebox; `PTY.ioctl.tiocgptn_only_on_master.pie-glibc.dpg1`, `PTY.devpts.dir_openable.pie-glibc.dpg1`, `PTY.readlink.proc_self_fd_stdio_returns_pty_path.pie-glibc.dpg1`, `PTY.slave_reopen.write_reaches_master.pie-glibc.dpg1`, `PTY.child_slave_reopen.write_reaches_master.pie-glibc.dpg1` (deterministic minimal probes — three shim divergences captured and fixed; two confirm broker dup'd-slave write→master works in simpler shapes) | **Regression window**: `708fd18..33a2453e`. Even baseline `tui.simple_math` / `tui.simple_bash` fail — TUI mode itself does not function under litebox independent of any shell-tool pipeline. Surfaced by Goal A validation session 7c1fc95d against amalgamation `898af46a` (`.dashboard/results.sqlite` run_id 30281). **Three shim divergences identified and fixed this session**: (a) `TIOCGPTN` on a PTY slave fd returned `pty_id()` instead of `-ENOTTY` (probe `076216a5`, fix `7858a614`); (b) `openat("/dev/pts/", O_DIRECTORY)` returned `ENOENT` instead of falling through to the rootfs FS (probe `8d5218b4`, fix `8c373d50`); (c) `readlink("/proc/self/fd/{0,1,2}")` returned `/dev/std{in,out,err}` placeholders instead of `/dev/pts/<pty_id>` when stdio was a `BrokerPty` slave (probe `a725f425`, fix `b65b6047`). All probes PASS native and PASS litebox. **However, `copilot::tui_noLLM.startup_then_exit` still hangs identically.** Post-fix audit log shows copilot successfully opens `/dev/pts/1` (the broker slave) and writes 78+166+81 bytes of TUI escapes to it — but those writes never reach the harness PTY master. The fourth divergence is in **dropbear's session-handler lifecycle under litebox**: worker 27 (dropbear auth/setup) opens `/dev/ptmx` as fd 6, does some setup, and exits at seq 576 — before any TUI rendering happens; subsequent slave writes succeed at the broker level but the harness sees nothing because no worker is actively reading the master and forwarding to SSH. Two-process and child-via-PTY-fork probes both PASS (`PTY.slave_reopen.*` + `PTY.child_slave_reopen.*`), so the broker's dup'd-slave write→master data path works in simpler shapes; the bug is specific to dropbear's multi-process session structure under litebox. Investigation continues. **Blocks** the W8/W10 row below: `find_head` pipeline behavior can't be validated until TUI mode itself starts. |
