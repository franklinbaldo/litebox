...
/// The platform capabilities the OP-TEE shim requires.
///
...
/// physical memory through [`NormalWorldConstPtr`]/[`NormalWorldMutPtr`]. Stating it here
/// keeps that requirement in the type system rather than in a compile-time platform
/// selection.
...
impl<Platform: OpteeShimPlatform> OpteeShimBuilder<Platform> {
    ...
    ///
    /// The platform is supplied by the composition root (the runner) rather than
    /// resolved from a compile-time-selected global.
    ///
    /// # Invariant
    ///
    ...
    /// registry splits session state across two views and lookups silently miss. This
    /// mis-keys a shared resource rather than merely partitioning one: session ids come
    /// from `SessionIdPool`, a single shim-owned pool, so a registry that never saw an
    /// id is indistinguishable from one whose session legitimately closed. Reach the
    /// registry via [`OpteeShim::session_manager`] where a shim is already in hand.
    pub fn new(
        platform: &'static Platform,
        session_manager: &'static session::SessionManager<Platform>,
    ) -> Self {
...
struct GlobalState<Platform: OpteeShimPlatform> {
    ...
    platform: &'static Platform,
    /// The session registry, owned by the composition root (the runner) because a
    /// `static` cannot name a generic parameter and a shim is built per session.
    session_manager: &'static session::SessionManager<Platform>,
