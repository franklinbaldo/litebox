...
// implies `Platform: Sync` (via `RawSyncPrimitivesProvider`), and everything `GlobalState`
// holds (`LiteBox`, `PageManager`, the handle maps) is auto-`Send + Sync`. `Platform`'s
// associated pointer types are neither, but nothing under `TaInstance` stores one: `Task`
// keeps addresses as `Cell<usize>`.
