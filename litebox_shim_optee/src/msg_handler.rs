...
/// and parameters), it contains shared memory transfers used for deferred access
/// and for writing output data back to the normal world.
...
/// The direction of a memref parameter, as requested by the normal world.
...
/// A memref parameter's normal-world shared memory, retained for the duration of a TA
/// request so that its payload can be copied directly to/from TA memory.
///
/// `buffer_size` is the byte size the normal world requested, which is always within
/// the bounds of `shm_info`.
...
///
/// Value parameters are copied into `UteeParamOwned` structures. Memref parameters are
/// *not* copied here: their payloads stay in the normal world and only the shared memory
/// metadata is retained in `memref_transfers`, so that the payload can later be copied
/// once, directly into the TA buffer, by [`materialize_deferred_inputs`]. The
/// corresponding `params` entry is left as `UteeParamOwned::None` for input and inout
/// memrefs; see [`deferred_memrefs`].
...
/// Copy deferred input and inout payloads directly into their final TA buffers.
///
/// `destinations` must be the buffers reserved by `load_ta_context` for the same
/// request, as described by [`deferred_memrefs`]. Every deferred memref must have a
/// matching reserved buffer of exactly the requested size, and no buffer may be
/// reserved for an output-only memref; otherwise `EBadAddr` is returned and the TA
/// must not be entered.
...
/// Describe the TA-stack buffers that `load_ta_context` must reserve for `request`.
///
/// Input and inout memrefs are reserved but left empty here; their payloads are copied
/// in by [`materialize_deferred_inputs`] once the buffer addresses are known. Output
/// memrefs are not deferred: they carry no input data and are already described by
...
/// the `OpteeMsgArgs` structure to return value-based outputs. Memref outputs are copied
/// directly from TA memory into the normal-world shared memory.
///
...
/// shared memory to write back output data to.
///
/// Note that `ta_params` is indexed by TA parameter index while `msg_args` is indexed by wire
/// parameter index; for `OpenSession` the two differ by the two leading meta parameters.
    ...
    for index in 0..UteeParams::TEE_NUM_PARAMS {
        ...
        match param_type {
            ...
            TeeParamType::MemrefOutput | TeeParamType::MemrefInout => {
                if let Ok(Some((addr, len_u64))) = ta_params.get_values(index) {
                    let Some(transfer) = &ta_req_info.memref_transfers[index] else {
                        continue;
                    };
                    ...
                    // ShortBuffer reports the TA-provided required size without applying the
                    // local memref allocation limit. Other oversized results return before the
                    // updated message is serialized to VTL0.
                    msg_args.set_param_memref_size(wire_index, len_u64)?;
                    if len_u64 > out_shm_info.size() as u64 {
                        if return_code == TeeResult::ShortBuffer {
                            continue;
                        }
                        return Err(OpteeSmcReturnCode::EBadAddr);
                    }
                    let len: usize = len_u64.trunc();
                    // SAFETY
...
impl<const ALIGN: usize> ShmInfo<ALIGN> {
    ...
    /// Copy shared memory directly into TA user memory.
    pub fn copy_to_user(
        &self,
    ...
    /// Copy TA user memory directly into shared memory.
